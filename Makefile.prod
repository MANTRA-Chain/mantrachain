...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
MANTRACHAIN_BINARY = mantrachaind
EVMOS_DIR = mantrachain
BUILDDIR ?= $(CURDIR)/build
HTTPS_GIT := https://github.com/LimeChain/mantrachain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=mantrachain \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(MANTRACHAIN_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm mantrachain || true
	# create a new container from the latest image
	$(DOCKER) create --name mantrachain -t -i ${DOCKER_IMAGE}:latest mantrachain
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp mantrachain:/usr/bin/mantrachaind ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/LimeChain/mantrachain/types"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(MANTRACHAIN_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/mantrachain:Z mantrachain/node "./mantrachaind testnet init-files --v 4 -o /mantrachain --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\mantrachaind:/mantrachain\Z mantrachain/node "./mantrachaind tendermint unsafe-reset-all --home=/mantrachain"
	@docker run --rm -v $(CURDIR)\build\node1\mantrachaind:/mantrachain\Z mantrachain/node "./mantrachaind tendermint unsafe-reset-all --home=/mantrachain"
	@docker run --rm -v $(CURDIR)\build\node2\mantrachaind:/mantrachain\Z mantrachain/node "./mantrachaind tendermint unsafe-reset-all --home=/mantrachain"
	@docker run --rm -v $(CURDIR)\build\node3\mantrachaind:/mantrachain\Z mantrachain/node "./mantrachaind tendermint unsafe-reset-all --home=/mantrachain"
else
	@docker run --rm -v $(CURDIR)/build/node0/mantrachaind:/mantrachain:Z mantrachain/node "./mantrachaind tendermint unsafe-reset-all --home=/mantrachain"
	@docker run --rm -v $(CURDIR)/build/node1/mantrachaind:/mantrachain:Z mantrachain/node "./mantrachaind tendermint unsafe-reset-all --home=/mantrachain"
	@docker run --rm -v $(CURDIR)/build/node2/mantrachaind:/mantrachain:Z mantrachain/node "./mantrachaind tendermint unsafe-reset-all --home=/mantrachain"
	@docker run --rm -v $(CURDIR)/build/node3/mantrachaind:/mantrachain:Z mantrachain/node "./mantrachaind tendermint unsafe-reset-all --home=/mantrachain"
endif
...
PACKAGE_NAME:=github.com/LimeChain/mantrachain

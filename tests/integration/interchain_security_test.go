...
func init() {
	// Set address prefixes to mantra
	...
	ccvSuite = integration.NewCCVTestSuite[*mantraApp.App, *appConsumer.App](
		// Pass in ibctesting.AppIniters for gaia (provider) and consumer.
		MantraAppIniterTempDir, icstestingutils.ConsumerAppIniter, []string{})
}
...
func TestICSEpochs(t *testing.T) {
	...
	provCtx = provCtx.WithBlockGasMeter(storetypes.NewInfiniteGasMeter())
	...
	endBlockerCtx := provCtx.WithBlockGasMeter(storetypes.NewInfiniteGasMeter())
	_, err := app.EndBlocker(endBlockerCtx)
	require.NoError(t, err)

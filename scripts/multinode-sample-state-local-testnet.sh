...
cecho "RED" "Do you want to mint sample nft collections and WUSDC for test usage? [y/N]"
read -r mint
...
"$PWD"/build/mantrachaind tx bridge create-cw-20-contract 0 "1" "./contracts/artifacts/v1/cw20_base-aarch64.wasm" --from admin1 --chain-id $CHAINID --keyring-backend $KEYRING --gas=auto --gas-adjustment="1.3" --gas-prices="$GAS_PRICE" --home $HOMEDIR/$KEY_VAL_1 --yes
...
"$PWD"/build/mantrachaind tx vault create-chain-validator-bridge polygon test $BRIDGE_1_WALLET id1 --chain-id $CHAINID --from admin1 --keyring-backend $KEYRING --gas auto --gas-adjustment $GAS_ADJ --gas-prices $GAS_PRICE --home $HOMEDIR/$KEY_VAL_1 --yes

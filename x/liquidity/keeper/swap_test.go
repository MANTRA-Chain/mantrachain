...
func (s *KeeperTestSuite) TestRangedLiquidity() {
	...
	s.Require().True(sdk.NewDecFromInt(paid).QuoInt(received).LTE(orderPrice))
	liquidity.BeginBlocker(s.ctx, s.keeper)
...
func (s *KeeperTestSuite) TestPoolPreserveK() {
	...
	for _, pool := range pools {
		...
		ks[pool.Id] = sdk.NewDecFromInt(rx.Amount).Add(transX).Mul(sdk.NewDecFromInt(ry.Amount).Add(transY))
	}
	...
	for i := 0; i < 20; i++ {
		...
		for _, pool := range pools {
			...
			ammPool := pool.AMMPool(rx.Amount, ry.Amount, math.Int{}).(*amm.RangedPool)
			transX, transY := ammPool.Translation()
			k := sdk.NewDecFromInt(rx.Amount).Add(transX).Mul(sdk.NewDecFromInt(ry.Amount).Add(transY))

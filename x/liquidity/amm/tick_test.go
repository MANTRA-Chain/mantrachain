...
func TestTick(t *testing.T) {
	for _, tc := range []struct {
		...
		{0, 3, math.LegacyNewDecWithPrec(1, int64(math.LegacyPrecision-defTickPrec))},
...
func TestHighestTick(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			require.True(math.LegacyDecEq(t, math.LegacyNewDecFromBigInt(i), tick))
		})
	}
}

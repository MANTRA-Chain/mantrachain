...
func TestCreateBasicPool(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			"too low price",
			sdk.NewInt(1000000), math.NewIntWithDecimal(1, 26),
			"pool price is lower than min price 0.000000000000001000",
		},
		{
			"too high price",
			math.NewIntWithDecimal(1, 48), sdk.NewInt(1000000),
...
func TestValidateRangedPoolParams(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			"too high max price",
			utils.ParseDec("0.5"), sdk.NewDecFromInt(math.NewIntWithDecimal(1, 25)),
			utils.ParseDec("1.0"),
...
func TestCreateRangedPool(t *testing.T) {
	intApproxEq := func(exp, got sdk.Int) (*testing.T, bool, string, string, string) {
		...
		if c && !exp.IsZero() {
			c = sdk.NewDecFromInt(exp).Sub(sdk.NewDecFromInt(got)).Abs().Quo(sdk.NewDecFromInt(exp)).LTE(sdk.NewDecWithPrec(1, 3))
		}
	...
	for _, tc := range []struct {
		...
		},
		{
			"large max price",
			sdk.NewInt(1_000000000000000000), sdk.NewInt(1_000000000000000000),
			utils.ParseDec("1.0"), sdk.NewDecFromInt(math.NewIntWithDecimal(1, 20)),
			utils.ParseDec("2.0"),
			"",
...
func TestRangedPool_Deposit(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			switch {
			case tc.rx.IsZero():
				reserveRatio = sdk.NewDecFromInt(ay).Quo(sdk.NewDecFromInt(tc.ry))
			case tc.ry.IsZero():
				reserveRatio = sdk.NewDecFromInt(ax).Quo(sdk.NewDecFromInt(tc.rx))
			default:
				reserveRatio = sdk.NewDecFromInt(ax).Quo(sdk.NewDecFromInt(tc.rx))
				require.True(t, utils.DecApproxEqual(reserveRatio, sdk.NewDecFromInt(ay).Quo(sdk.NewDecFromInt(tc.ry))))
			}

			// check ax/ay == rx/ry
			if !tc.rx.IsZero() && !tc.ry.IsZero() {
				require.True(t, utils.DecApproxEqual(sdk.NewDecFromInt(ax).Quo(sdk.NewDecFromInt(ay)), sdk.NewDecFromInt(tc.rx).Quo(sdk.NewDecFromInt(tc.ry))))
			}

			// check ax/rx == ay/ry == pc/ps
			require.True(t, utils.DecApproxEqual(reserveRatio, sdk.NewDecFromInt(pc).Quo(sdk.NewDecFromInt(tc.ps))))
...
func TestRangedPool_Withdraw(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			switch {
			case tc.rx.IsZero():
				reserveRatio = sdk.NewDecFromInt(y).Quo(sdk.NewDecFromInt(tc.ry))
			case tc.ry.IsZero():
				reserveRatio = sdk.NewDecFromInt(x).Quo(sdk.NewDecFromInt(tc.rx))
			default:
				reserveRatio = sdk.NewDecFromInt(x).Quo(sdk.NewDecFromInt(tc.rx))
				require.True(t, utils.DecApproxEqual(reserveRatio, sdk.NewDecFromInt(y).Quo(sdk.NewDecFromInt(tc.ry))))
			}

			// check x/y == rx/ry
			if !tc.rx.IsZero() && !tc.ry.IsZero() {
				require.True(t, utils.DecApproxEqual(sdk.NewDecFromInt(x).Quo(sdk.NewDecFromInt(y)), sdk.NewDecFromInt(tc.rx).Quo(sdk.NewDecFromInt(tc.ry))))
			}

			// check x/rx == y/ry == pc/ps
			require.True(t, utils.DecApproxEqual(reserveRatio, sdk.NewDecFromInt(tc.pc).Quo(sdk.NewDecFromInt(tc.ps))))
...
func TestRangedPool_exhaust(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			require.True(t, amt.GTE(sdk.NewDecFromInt(ry).Mul(utils.ParseDec("0.99")).TruncateInt()))
			...
			require.True(t, x.GTE(sdk.NewDecFromInt(rx).Mul(utils.ParseDec("0.99")).TruncateInt()))
		})
	}
}

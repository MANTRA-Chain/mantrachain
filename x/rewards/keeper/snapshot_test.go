...
func TestSnapshotGet(t *testing.T) {
	...
	for _, item := range items {
		got, found := keeper.GetSnapshot(ctx, item.Id)
...
func TestSnapshotRemove(t *testing.T) {
	...
	items := createNSnapshot(keeper, ctx, 10)
	for _, item := range items {
		keeper.RemoveSnapshot(ctx, item.Id)
		_, found := keeper.GetSnapshot(ctx, item.Id)
...
func TestSnapshotGetAll(t *testing.T) {
	...
	require.ElementsMatch(t,
		nullify.Fill(items),
		nullify.Fill(keeper.GetAllSnapshot(ctx)),
	)
}
...
func TestSnapshotCount(t *testing.T) {
	...
	require.Equal(t, count, keeper.GetSnapshotCount(ctx))
}

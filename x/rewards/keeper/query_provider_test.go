...
func TestProviderPairsQuery(t *testing.T) {
	keeper, ctx := keepertest.RewardsKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	tests := []struct {
		desc     string
		request  *types.QueryGetProviderPairsRequest
		response *types.QueryGetProviderPairsResponse
		err      error
	}{
		{
			desc: "InvalidRequest",
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.ProviderPairs(wctx, tc.request)
			if tc.err != nil {

...
func networkWithSnapshotObjects(t *testing.T, n int) (*network.Network, []types.Snapshot) {
	...
	return network.New(t, cfg), state.SnapshotList
}

func TestShowSnapshot(t *testing.T) {
	net, objs := networkWithSnapshotObjects(t, 2)
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowSnapshot(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetSnapshotResponse
...
func TestListSnapshot(t *testing.T) {
	net, objs := networkWithSnapshotObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListSnapshot(), args)
			require.NoError(t, err)
			var resp types.QueryAllSnapshotResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))

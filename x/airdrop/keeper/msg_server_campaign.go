...
func (k msgServer) CreateCampaign(goCtx context.Context, msg *types.MsgCreateCampaign) (*types.MsgCreateCampaignResponse, error) {
	...
	whitelisted := k.guardKeeper.AddTransferAccAddressesWhitelist(ctx, []sdk.AccAddress{campaign.GetReserveAddress()})
	if err != nil {
		return nil, err
	}
...
func (k Keeper) TerminateCampaign(ctx sdk.Context, campaign types.Campaign) error {
	...
	if !balances.IsZero() {
		whitelisted := k.guardKeeper.AddTransferAccAddressesWhitelist(ctx, []sdk.AccAddress{campaignAddr})

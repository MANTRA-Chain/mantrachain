...
func (s *KeeperTestSuite) TestCreatePrivatePlan_TooManyPrivatePlans() {
	s.createPairWithLastPrice("denom1", "denom2", math.LegacyNewDec(1))
	s.createPairWithLastPrice("denom2", "denom3", math.LegacyNewDec(1))
...
func (s *KeeperTestSuite) TestAllocateRewards() {
	...
	farm, _ = s.keeper.GetFarmFromStore(s.ctx, "pool2")
...
func (s *KeeperTestSuite) TestAllocatedRewards_Complicated() {
	...
	s.createRangedPool(
		...
		utils.ParseDec("0.8"), utils.ParseDec("1.25"), math.LegacyNewDec(1))
	s.createRangedPool(
		1, utils.ParseCoins("100_000000denom1,100_000000denom2"),
		utils.ParseDec("0.4"), utils.ParseDec("2.5"), math.LegacyNewDec(1))
	...
	s.createPairWithLastPrice("denom3", "denom4", math.LegacyNewDec(1))
	s.createRangedPool(
	...
	s.createPairWithLastPrice("denom4", "denom5", math.LegacyNewDec(1))
	s.createRangedPool(
...
func (s *KeeperTestSuite) TestPreviousShare() {
	...
	farm, _ = s.keeper.GetFarmFromStore(s.ctx, "pool3")
	...
	farm, _ = s.keeper.GetFarmFromStore(s.ctx, "pool1")
	s.assertEq(math.LegacyNewDec(1), *farm.PreviousShare)
	farm, _ = s.keeper.GetFarmFromStore(s.ctx, "pool2")
	s.Require().Nil(farm.PreviousShare)
	farm, _ = s.keeper.GetFarmFromStore(s.ctx, "pool3")
	s.Require().Nil(farm.PreviousShare)

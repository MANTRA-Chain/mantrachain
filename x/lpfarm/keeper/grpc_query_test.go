...
func (s *KeeperTestSuite) TestGRPCFarm() {
	...
	_, _ = s.keeper.FarmLock(s.ctx, farmerAddr, utils.ParseCoin("1_000000pool1"))
	...
	for _, tc := range []struct {
	...
	}{
		{
			"nil request",
			nil,
			"rpc error: code = InvalidArgument desc = invalid denom: invalid denom: ",
			nil,
		},
		{
			"query a farm",
...
func (s *KeeperTestSuite) TestGRPCPositions() {
	...
	_, _ = s.keeper.FarmLock(s.ctx, farmerAddr, utils.ParseCoin("1_000000pool2"))
	...
	for _, tc := range []struct {
	...
	}{
		{
			"nil request",
			nil,
			"rpc error: code = InvalidArgument desc = invalid farmer address: empty address string is not allowed",
			nil,
		},
		{
...
func (s *KeeperTestSuite) TestGRPCRewards() {
	...
	for _, tc := range []struct {
		...
		s.Run(tc.name, func() {
			resp, err := s.queryClient.Rewards(s.ctx, tc.req)
			if tc.expectedErr == "" {
				s.Require().NoError(err)
				tc.postRun(resp)
			} else {

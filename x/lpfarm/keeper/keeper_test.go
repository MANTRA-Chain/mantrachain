...
func (s *KeeperTestSuite) fundAddr(addr sdk.AccAddress, amt sdk.Coins) {
	s.T().Helper()
	s.Require().NoError(s.app.BankKeeper.MintCoins(s.ctx, liquiditytypes.ModuleName, amt))
	s.Require().NoError(
		s.app.BankKeeper.SendCoinsFromModuleToAccount(s.ctx, liquiditytypes.ModuleName, addr, amt))
}

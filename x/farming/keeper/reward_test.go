...
func (suite *KeeperTestSuite) TestAllocateRewards_FixedAmountPlanAllBalances() {
	farmingPoolAcc := testutil.AddTestAddrs(suite.app, suite.ctx, 1, sdk.ZeroInt())[0]
	err := testutil.FundAccount(suite.app.BankKeeper, suite.ctx, farmingPoolAcc, sdk.NewCoins(sdk.NewInt64Coin(denom3, 1000000)))
	suite.Require().NoError(err)

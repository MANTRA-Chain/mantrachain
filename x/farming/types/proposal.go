...
func (p *PublicPlanProposal) ValidateBasic() error {
	if len(p.AddPlanRequests) == 0 && len(p.ModifyPlanRequests) == 0 && len(p.DeletePlanRequests) == 0 {
		return errors.Wrap(errorstypes.ErrInvalidRequest, "proposal request must not be empty")
	}
...
func (p *AddPlanRequest) Validate() error {
	...
	if err != nil {
		return errors.Wrapf(errorstypes.ErrInvalidAddress, "invalid farming pool address %q: %v", p.FarmingPoolAddress, err)
	}
	if _, err := sdk.AccAddressFromBech32(p.TerminationAddress); err != nil {
		return errors.Wrapf(errorstypes.ErrInvalidAddress, "invalid termination address %q: %v", p.TerminationAddress, err)
	}
	...
	case isForFixedAmountPlan == isForRatioPlan:
		return errors.Wrap(errorstypes.ErrInvalidRequest, "exactly one of epoch amount or epoch ratio must be provided")
...
func (p *ModifyPlanRequest) Validate() error {
	if p.PlanId == 0 {
		return errors.Wrapf(errorstypes.ErrInvalidRequest, "invalid plan id: %d", p.PlanId)
	}
	if p.Name != "" {
	...
	switch {
	case isForFixedAmountPlan && isForRatioPlan:
		return errors.Wrap(errorstypes.ErrInvalidRequest, "at most one of epoch amount or epoch ratio must be provided")
	case isForFixedAmountPlan:

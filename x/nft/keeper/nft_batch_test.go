...
func (s *TestSuite) TestBatchMint() {
	...
	testCases := []struct {
		msg      string
		malleate func([]types.NFT)
		tokens   []types.NFT
		expPass  bool
	}{
		{
			"success with empty nft",
			func(tokens []types.NFT) {
				s.saveClass(tokens)
			},
			[]types.NFT{},
			true,
...
func groupByClassID(tokens []types.NFT) map[string][]types.NFT {
	classMap := make(map[string][]types.NFT, len(tokens))
	for _, token := range tokens {
		if _, ok := classMap[token.ClassId]; !ok {
			classMap[token.ClassId] = make([]types.NFT, 0)
		}
...
func (s *TestSuite) saveClass(tokens []types.NFT) {
	classMap := groupByClassID(tokens)
	for classID := range classMap {
		err := s.nftKeeper.SaveClass(s.ctx, types.Class{Id: classID})
		s.Require().NoError(err)
	}
}

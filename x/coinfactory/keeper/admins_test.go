...
func (suite *KeeperTestSuite) TestAdminMsgs() {
	...
	_, err = suite.msgServer.ForceTransfer(sdk.WrapSDKContext(suite.ctx), types.NewMsgForceTransfer(suite.TestAccs[0].String(), sdk.NewInt64Coin(suite.defaultDenom, 5), suite.TestAccs[0].String(), suite.TestAccs[1].String()))
	addr0bal -= 5
	addr1bal += 5
	suite.Require().NoError(err)
	suite.Require().True(bankKeeper.GetBalance(suite.ctx, suite.TestAccs[0], suite.defaultDenom).IsEqual(sdk.NewInt64Coin(suite.defaultDenom, addr0bal)))
	suite.Require().True(bankKeeper.GetBalance(suite.ctx, suite.TestAccs[1], suite.defaultDenom).IsEqual(sdk.NewInt64Coin(suite.defaultDenom, addr1bal)))

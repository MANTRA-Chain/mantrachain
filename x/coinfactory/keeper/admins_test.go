...
func (suite *KeeperTestSuite) TestAdminMsgs() {
	...
	_, err = suite.msgServer.Mint(suite.ctx, types.NewMsgMint(suite.addrs[0].String(), sdk.NewInt64Coin(suite.defaultDenom, 10)))
	addr0bal += 10
	suite.Require().NoError(err)
	...
	_, err = suite.msgServer.ForceTransfer(suite.ctx, types.NewMsgForceTransfer(suite.addrs[0].String(), sdk.NewInt64Coin(suite.defaultDenom, 5), suite.addrs[0].String(), suite.addrs[1].String()))
	addr0bal -= 5
	addr1bal += 5
	suite.Require().NoError(err)
	...
	_, err = suite.msgServer.ForceTransfer(suite.ctx, types.NewMsgForceTransfer(suite.addrs[0].String(), sdk.NewInt64Coin(suite.defaultDenom, 5), suite.addrs[1].String(), suite.addrs[0].String()))
	addr0bal += 5
	addr1bal -= 5
	suite.Require().NoError(err)
	suite.Require().True(bankKeeper.GetBalance(suite.ctx, suite.addrs[0], suite.defaultDenom).IsEqual(sdk.NewInt64Coin(suite.defaultDenom, addr0bal)))
	...
	_, err = suite.msgServer.Burn(suite.ctx, types.NewMsgBurn(suite.addrs[0].String(), sdk.NewInt64Coin(suite.defaultDenom, 5)))
	suite.Require().NoError(err)
	suite.Require().True(bankKeeper.GetBalance(suite.ctx, suite.addrs[1], suite.defaultDenom).Amount.Int64() == addr1bal)

	// Test Change Admin
	_, err = suite.msgServer.ChangeAdmin(suite.ctx, types.NewMsgChangeAdmin(suite.addrs[0].String(), suite.defaultDenom, suite.addrs[1].String()))
	suite.Require().NoError(err)
	queryRes, err = suite.queryClient.DenomAuthorityMetadata(suite.ctx.Context(), &types.QueryDenomAuthorityMetadataRequest{
		Denom: suite.defaultDenom,
	})
	...
	_, err = suite.msgServer.Mint(suite.ctx, types.NewMsgMint(suite.addrs[1].String(), sdk.NewInt64Coin(suite.defaultDenom, 5)))
	addr1bal += 5
	suite.Require().NoError(err)
	suite.Require().True(bankKeeper.GetBalance(suite.ctx, suite.addrs[1], suite.defaultDenom).Amount.Int64() == addr1bal)

	// Try setting admin to empty
	_, err = suite.msgServer.ChangeAdmin(suite.ctx, types.NewMsgChangeAdmin(suite.addrs[1].String(), suite.defaultDenom, ""))
	suite.Require().NoError(err)
	queryRes, err = suite.queryClient.DenomAuthorityMetadata(suite.ctx.Context(), &types.QueryDenomAuthorityMetadataRequest{
		Denom: suite.defaultDenom,
	})
...
func (suite *KeeperTestSuite) TestChangeAdminDenom() {
	for _, tc := range []struct {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			_, err = suite.msgServer.Mint(suite.ctx, types.NewMsgMint(suite.addrs[0].String(), sdk.NewInt64Coin(testDenom, 10)))
			suite.Require().NoError(err)

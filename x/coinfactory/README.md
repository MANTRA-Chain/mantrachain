...
# Examples
To create a new token, use the create-denom command from the coinfactory module. The following example uses the address mantrachain1c584m4lq25h83yp6ag8hh4htjr92d954vklzja from mylocalwallet as the default admin for the new token.

## Creating a token
To create a new token we can use the create-denom command.

```sh
mantrachaind tx coinfactory create-denom ufoo --keyring-backend=test --from mylocalwallet
```

## Mint a new token
Once a new token is created, it can be minted using the mint command in the coinfactory module. Note that the complete coinfactory address, in the format of factory/{creator address}/{subdenom}, must be used to mint the token.

```sh
mantrachaind tx coinfactory mint 100000000000factory/mantrachain1c584m4lq25h83yp6ag8hh4htjr92d954vklzja/ufoo --keyring-backend=test --from mylocalwallet
```

## Checking Token metadata
To view a token's metadata, use the denom-metadata command in the bank module. The following example queries the metadata for the token factory/mantrachain1c584m4lq25h83yp6ag8hh4htjr92d954vklzja/ufoo:

```sh
mantrachaind query bank denom-metadata --denom factory/mantrachain1c584m4lq25h83yp6ag8hh4htjr92d954vklzja/ufoo
```

## Check the tokens created by an account
To see a list of tokens created by a specific account, use the denoms-from-creator command in the coinfactory module. The following example shows tokens created by the account mantrachain1c584m4lq25h83yp6ag8hh4htjr92d954vklzja:

```sh
mantrachaind query coinfactory denoms-from-creator mantrachain1c584m4lq25h83yp6ag8hh4htjr92d954vklzja
```
